...
<section class="promotions">
    ...
    <div class="promo-grid" id="promoGrid">
        ...
        <div class="promo-card" data-code="{{ $promo['code'] }}"
            data-airline="{{ $promo['airline_name'] }}"
            data-end="{{ strtotime($promo['end_date']) }}">
            <img src="{{ asset($promo['logo_url']) }}"
                alt="{{ $promo['airline_name'] }}">
            <div class="promo-info">
                <h3>{{ $promo['airline_name'] }} - Mã: <span
                        class="code">{{ $promo['code'] }}</span></h3>
                ...
                <button class="btn-copy" onclick="copyCode('{{ $promo['code'] }}')">Sao chép
                    mã</button>
            </div>
        </div>
        @endforeach
